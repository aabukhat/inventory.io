 ...
 * and regenerates the liquor entries in the `products` table (brand/flavor
 * stay null for these — no reliable way to split them, see CLAUDE.md).
 *
 * The catalog lives in the database now, not in src/lib/products.js, so
 * this script emits a reviewable SQL file rather than writing anything
 * itself — apply it the same way every other schema/data change in this
 * repo is applied:
 *   op run --env-file=.env.local -- npx supabase db query --linked --file <path>
 *
 * Beer/seltzer/cider and manually-added liquor rows (is_generated = false)
 * are untouched — this only replaces rows where is_generated = true.
 *
...
const OUTPUT_PATH = path.join(__dirname, '..', 'liquor-regen-seed.sql')
...
// ─── Sort by category then name, for a clean, diff-friendly SQL file ─────────
...
// ─── Emit a reviewable SQL file replacing the generated liquor rows ──────────
function sqlStr(v) {
  return `'${String(v).replace(/'/g, "''")}'`
}

function emitSql(products) {
  const lines = [
    '-- Auto-generated by scripts/generate-products.js — review before applying.',
    "delete from public.products where type = 'liquor' and is_generated = true;",
    'insert into public.products (brand, drink_name, flavor, type, default_unit, default_size, is_generated) values',
  ]
  const rows = organise(products).map(({ name, type, defaultUnit, defaultSize }) =>
    `(null, ${sqlStr(name)}, null, ${sqlStr(type)}, ${sqlStr(defaultUnit)}, ${sqlStr(defaultSize)}, true)`
  )
...
console.log('  op run --env-file=.env.local -- npx supabase db query --linked --file liquor-regen-seed.sql')
