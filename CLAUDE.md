...
- **`inventories`** — `name`, `type` (`'personal'|'shared'`, default `'shared'`), `owner_id`. Unique partial index: exactly one `type='personal'` row per `owner_id`.
